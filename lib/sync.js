...
/**
  @function writeFile(p, data[, options]) write file to the path(do not care whether the path exists or not)
...
/**
  @function walk(p, fn[, regexp]) treat all files with fn.(if dir is true treat all files and directories) 
...
/**
  @function watch(p, fn[, interval]) watch all files an directories in the path 
  @param p {string|array[string]} path[path array, directory, directory array]
